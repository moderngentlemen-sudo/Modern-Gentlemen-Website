...
- [ ] Signed-in E2E (`tests/e2e/builder.spec.ts`) **written but not executed in this container** (no Docker daemon, so no local stack). Its first real run is the CI run on this branch — see "Verification honesty" below
