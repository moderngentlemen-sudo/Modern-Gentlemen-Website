...
- [ ] **The visual job has still never compared a baseline on a CI host.** `test:visual` runs after `test:e2e` in the same job, so every red e2e run so far has stopped short of it. It settles on the first fully green run, not before
