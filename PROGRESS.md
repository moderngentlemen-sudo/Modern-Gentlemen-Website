...
> ✅ **All of Track B is merged.** PR #7 landed on `main` as `987ba05` — 31 commits, Phases 0 through 4, with CI green on every job. `main` is the baseline again; Phase 5 starts from it. The working branch was restarted from the merged `main` rather than continuing on top of already-merged history.
>
...
- **CI is `concurrency: cancel-in-progress`, and the e2e job takes ~6 minutes.** Two pushes inside that window kill the first run mid-`supabase start`, which reads exactly like an infrastructure failure and is not one. Batch a code fix and its PROGRESS.md entry into **one** commit rather than pushing them separately.
- **`test:visual` runs after `test:e2e` in the same job**, so a red e2e means the visual baselines are never compared at all — the job goes red without the visual gate having run. If e2e is failing, no statement about visual regression is available yet.
...
1. **Phase 5** — CMS & media. Read the four items below *before* planning; each is something a session would otherwise discover halfway in.

   - **⚠️ No storage bucket exists.** `0002_media.sql` creates five tables — `media_folders`, `media_assets`, `media_tags`, `media_asset_tags`, `media_usages` — and `media_assets` defaults `bucket` to `'media'` with a `unique (bucket, storage_path)`. **Nothing has ever created that bucket.** Verified against the live project: `storage.buckets` is empty. No migration anywhere references `storage.buckets` or `storage.objects`, so there are no storage RLS policies either. Uploading is therefore blocked until a migration creates the bucket and its policies — that is the first task of the phase, not an afterthought.
   - **There is no media service or repository at all.** The `media.*` permissions (`read`/`write`/`delete`) exist in `lib/domain/permissions.ts` and the schema is thorough — checksum dedup, focal point, blur-up `placeholder`, a GIN search index over title/file_name/alt_text/caption — but no code has ever read or written any of it. Phase 5 is a real build, not a wiring job.
   - **`lib/blocks/traverse.ts#walkBlocks` is the walker** to reuse for media-usage extraction into `media_usages`; do not write a second one. Note `media_usages.field_path` is documented as "a block key, or a field path such as `hero.media`" and carries `unique (asset_id, entity_type, entity_id, field_path)` — reconciliation on save is an upsert-and-prune against that key, not an append.
   - **Phase 4 left the seam deliberately.** `image`/`video` fields render as URL inputs and `richText` as a textarea, in `components/admin/fields/`. Swapping in a media picker is a change to `MediaUrlControl` alone — the manifests, the store and the panel do not move. Across the 22 manifests that is 9 `image`, 2 `video`, 1 `richText`.

   Also carried in: the service-role key still needs rotating (it blocks provisioning an admin in a fresh container, which in turn blocks capturing the four admin visual baselines), and `templates`/`patterns` remain empty tables that no code path has ever exercised.
...
- **Phase 3 (documents, publishing, revisions, preview) — decisions.**
  ...
  - **`0012` deliberately says nothing about functions.** Postgres grants EXECUTE to PUBLIC on creation, which is exactly why `0009`/`0010`/`0011` close with *revokes* on the publishing functions. A blanket `grant execute on all functions` in `0012` would have silently undone them and handed an anonymous caller `publish_document`. **Confirmed after applying `0012` upstream**, which is the moment that claim stopped being an argument and became a fact: `anon` holds no EXECUTE on `publish_document`, `unpublish_document`, `rollback_document`, `snapshot_document`, `schedule_document` or `autosave_document`, and `authenticated` holds all six. `resolve_preview` is executable by `anon` — deliberately, since `/preview/[token]` serves an unauthenticated token holder.
  - **`0012` is applied to the live project** (`20260802055804`), so repo and hosted history both stand at 12. As predicted it changed nothing: the privileges it states were already held. Its value is that they are now *stated* — the next `supabase start` reproduces the hosted project instead of diverging from it.
...
- [ ] **The media schema has no storage behind it.** `0002_media.sql` builds five tables and `media_assets` defaults `bucket` to `'media'`, but no migration has ever created that bucket or any `storage.objects` policy — `storage.buckets` is empty on the live project. Nothing is broken today because no code touches media; it blocks the first upload in Phase 5.
