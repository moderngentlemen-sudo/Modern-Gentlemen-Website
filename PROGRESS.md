...
- **Performance pass (see `/PERFORMANCE.md` for the measurements).** Homepage 4.27 MB → **0.51 MB**, `/shop` 4.28 MB → **0.33 MB**, `/article/*` → **0.64 MB**. Four changes: (1) **the two hotlinked videos are unset** in `lib/media.ts` — the prototype's placeholders measured 38.8 MB (hero) and **1.9 GB** (a 4K/60fps Wikimedia clip the film row autoplayed into a 240px tile), and neither was ours to serve; every video path is intact and gated on a `videoUrl`, so both sections fall back to the still exactly as `ArticleHero`'s Film Feature already did, and film tiles are inert until footage exists. (2) **All 16 raster `<img>` → `next/image`** with `fill` + per-breakpoint `sizes`, `priority` on the hero and article covers, lazy elsewhere; the three SVG logos stay plain `<img>` (SVG needs `dangerouslyAllowSVG` and they're 3–10 KB). Containers that lacked `position:relative` gained it — required by `fill`, no layout effect. (3) **CSS `background-image` covers now go through `lib/bgImage.ts`** (`getImageProps`) — six components (`CategoryHero`, `FeaturedLead`, `StoryBand`, `ArticleGrid`, `RelatedGrid`, the `bg()` helper in `bodies-structured`) bypassed the optimizer entirely and were still shipping raw 4K; markup untouched, so the category page diffs 0.000%. (4) `useHideOnScroll` is rAF-coalesced with guarded setters, the header no longer **transitions** `backdrop-filter` (it re-ran a full-width blur every frame for 450ms), the search overlay drops the scrim blur it stacked under a full-viewport panel, and its own blur is disabled ≤820px — restoring the prototype's `mobileSearchBlur` opt-out the port had dropped. **Visual check:** 5 pages × 2 viewports, full-page, before/after on identical builds — all heights byte-identical, diffs 0.000–0.221% (WebP noise); the one 1.85% outlier is a Chromium full-page-capture artifact with lazily-decoded `next/image`, disproved by a viewport screenshot and a DOM probe.
