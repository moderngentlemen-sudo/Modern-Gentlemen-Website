...
**Branch:** `claude/modern-gentlemen-admin-ntslwo-ngmzu7` (all backend work). **Deploy:** Railway, Root Directory `design_handoff_modern_gentlemen/starter`. **Database:** Supabase project `qnfoztnyxhubnnulpfwt` — schema applied and seeded, **live**. Migrations `0001`–`0011`.
...
- **Phase 3 (documents, publishing, revisions, preview) — decisions.**
  ...
  - **⚠️ And CI does not close it either — correcting an earlier claim in this file.** `.github/workflows/ci.yml` starts a throwaway local Supabase stack but **never provisions an admin account and never exports `E2E_ADMIN_EMAIL` / `E2E_ADMIN_PASSWORD`**, and references no repository secrets at all. So every signed-in spec — the four in `auth.spec.ts` as well as all three in the new `builder.spec.ts` — **skips in CI**, and has done since Phase 1c. The signed-in path is therefore verified *nowhere*. Closing this is a CI change, not a test change: add a step that runs `scripts/create-admin.ts` against the local stack with a fixed `SEED_ADMIN_PASSWORD`, then export both variables into the e2e job. That is the single highest-value thing Phase 5 can do first, and it costs one workflow step.
...
- [ ] Signed-in E2E (`tests/e2e/builder.spec.ts`) **written but never executed — not here and not in CI**, which provisions no admin account. See "Verification honesty" below
- [ ] **The public visual baselines in `tests/visual/public.spec.ts-snapshots/` were captured in a dev container and CI compares them on `ubuntu-latest`.** Cross-host font antialiasing is exactly what `maxDiffPixelRatio: 0.01` was set for, but these are full-page shots of text-heavy editorial pages and the tolerance may not hold. If the visual job goes red on first run, re-baseline from the CI artifact rather than loosening the gate
