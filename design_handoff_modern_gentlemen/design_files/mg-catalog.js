/* Modern Gentlemen — shared product catalog (business data).
   Loaded as a plain <script> in each store page's <helmet>. Sets window.MGCatalog. */
(function () {
  var P = [
  ...
  window.MGCatalog = {
    ...
    byGroup: function (group) { return group === 'All' ? P.slice() : P.filter(function (p) { return p.cat === group; }); },
    related: function (slug, n) {
      var p = bySlug[slug]; if (!p) return P.slice(0, n || 4);
      var same = P.filter(function (x) { return x.cat === p.cat && x.slug !== slug; });
      var rest = P.filter(function (x) { return x.cat !== p.cat; });
      return same.concat(rest).slice(0, n || 4);
    },
    groups: ['All', 'Style', 'Watches', 'Grooming', 'Accessories'],
    format: function (n) { return '£' + Number(n).toLocaleString('en-GB'); }
  };
})();
