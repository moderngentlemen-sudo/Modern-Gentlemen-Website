...
- **Newsletter** — `/admin/newsletter`: permission-gated subscriber lifecycle/source view and formula-safe CSV export. Capture remains provider-neutral and pending until an owner-selected ESP implements double opt-in.
...
- The registered section catalogue includes every one of the 125 checked-in
  Section Library designs plus 20 additive platform modules. `Hero Studio` and
  `Section Studio` expose their numbered presets through shared native
  archetypes; `../MODULE_MAP.md` records provenance and the compatibility map.
