  ...
  return (
    <section>
      ...
      <div className="mx-4 min-[681px]:mx-[max(var(--layout-desktop-gutter),calc((100%-var(--layout-content-width))/2))]">
        ...
        >
          {eyebrow && (
            <Eyebrow className="block !text-[24px] !leading-[normal] mb-4">{eyebrow}</Eyebrow>
          )}
          {/* ≤680 the band's heading drops to 26px but keeps the global mobile
              section-heading leading of 1.05, not its own 1.15. */}
          <h2 className="mx-auto max-w-[800px] font-grotesk font-medium text-[26px] leading-[1.05] min-[681px]:text-[44px] min-[681px]:leading-[1.15] tracking-[-0.03em]">
            {quote}
          </h2>
          {body && (
            <p className="mx-auto mt-[22px] max-w-[620px] font-light text-[15px] min-[681px]:text-[16px] leading-[1.75] text-[#f4f4f4]/55">
              {body}
            </p>
          )}
          {attribution && (
