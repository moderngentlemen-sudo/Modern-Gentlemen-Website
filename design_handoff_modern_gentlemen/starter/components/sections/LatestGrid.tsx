...
export function LatestGrid({
  ...
  const cols =
    variant === "featureLeft"
      ...
      : variant === "mosaic"
        ? "min-[681px]:grid-cols-4"
        : "min-[681px]:grid-cols-3";
  return (
  ...
  return (
    <div className="flex items-baseline justify-between gap-4 mb-[22px]">
      <div>
        ...
        <h2 className="mt-1 font-grotesk font-semibold text-3xl leading-[1.05] min-[681px]:text-[42px] min-[681px]:leading-none tracking-[-0.035em]">
          {heading}
        </h2>
      </div>
      {viewAllHref && (
...
function Tile({ item }: { item: Item }) {
  if (item.kind === "feature") {
    const inner = (
      ...
      >
        ...
        <h3 className="font-grotesk font-medium text-[30px] leading-[1.06] tracking-[-0.03em]">
          {item.title}
        </h3>
        {item.body && (
    ...
    return item.href ? (
      <Link href={item.href} className={TILE_PADDED_RULED}>
        {inner}
      </Link>
    ) : (
      inner
  ...
  if (item.kind === "membership") {
    const inner = (
      <div className="flex w-full flex-col justify-between bg-mg-accent text-white px-[30px] py-7 transition-transform duration-200 hover:-translate-y-[3px]">
        ...
        <h3 className="font-grotesk font-medium text-[26px] leading-[1.08] tracking-[-0.025em]">
          {item.title}
        </h3>
        {item.body && (
    ...
    return item.href ? (
      <Link href={item.href} className={TILE_PADDED}>
        {inner}
      </Link>
    ) : (
      inner
  ...
  const inner = (
    <article className="group relative w-full overflow-hidden bg-mg-surface transition-transform duration-200 hover:-translate-y-[3px]">
      ...
      <div
        ...
        style={{
          background: "linear-gradient(180deg,rgba(13,13,13,0) 45%,rgba(13,13,13,0.35) 100%)",
        }}
      />
      <div
        className="absolute inset-x-0 bottom-0 px-[18px] py-[14px] border-t border-white/15"
        style={{
      ...
      >
        ...
        <h3 className="mt-1.5 font-grotesk font-medium text-white text-[19px] leading-[1.15] tracking-[-0.02em]">
          {item.title}
        </h3>
      </div>
    </article>
  ...
  return item.href ? (
    <Link href={item.href} className={TILE_PLAIN}>
      {inner}
    </Link>
  ) : (
    inner
