...
export function ProductRow({ heading = "The Store", eyebrow, group, slugs, href = "/shop" }: {
  heading?: string; eyebrow?: string; group?: string; slugs?: string[]; href?: string;
}) {
  const cart = useCart();
  ...
  return (
    <section className="container-mg py-16 md:py-24">
      <div className="flex items-end justify-between mb-7">
        ...
        <Link href={href} className="font-mono text-[11px] tracking-[0.18em] text-mg-accent">STORE ALL →</Link>
      </div>
        ...
        {products.map((p) => p && (
          <div key={p.slug} className="group">
            ...
            <div className="mt-3 flex items-baseline justify-between gap-2">
              <Link href={`/product/${p.slug}`} className="font-grotesk text-sm">{p.name}</Link>
              <span className="font-mono text-sm shrink-0">{formatGBP(p.price)}</span>
            </div>
            <button onClick={() => cart.add(p.slug)} className={`mt-2 w-full py-2 font-mono text-[10px] uppercase tracking-[0.15em] border ${cart.has(p.slug) ? "border-mg-accent text-mg-accent" : "border-mg-bd/25 hover:bg-mg-fg hover:text-mg-bg"}`}>
