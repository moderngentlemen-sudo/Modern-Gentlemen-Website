...
export function Interview({ eyebrow, headline, subject, qa }: {
  eyebrow?: string; headline: string; subject?: string; qa: QA[];
}) {
  return (
    <section className="container-mg py-16 md:py-24 max-w-3xl">
      ...
      <h2 className="font-grotesk font-semibold text-3xl md:text-5xl mt-2 text-balance">{headline}</h2>
      {subject && <p className="mt-3 font-mono text-xs uppercase tracking-[0.18em] text-mg-fg/50">{subject}</p>}
