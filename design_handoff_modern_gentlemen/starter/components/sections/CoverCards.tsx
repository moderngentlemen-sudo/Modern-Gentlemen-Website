...
export function CoverCards({ label, cards }: { label: string; cards: Card[] }) {
  return (
    <section className="pt-20" style={{ paddingInline: "max(22px, calc((100% - 1320px) / 2))" }}>
      ...
      <div className="grid grid-cols-1 gap-[20px] min-[821px]:grid-cols-3">
        {cards.map((c) => (
          <Link
            key={c.href}
            href={c.href}
            className="block rounded-[12px] border border-mg-bd/10 bg-mg-surface p-[32px_30px] transition-[transform,border-color] duration-200 hover:-translate-y-1 hover:border-mg-accent/50"
          >
