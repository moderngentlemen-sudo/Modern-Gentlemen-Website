    ...
    return (
      <section data-darkband className="relative h-[560px] bg-[#0d0d0d] text-white overflow-hidden">
        ...
        <div
          ...
          style={{ background: "linear-gradient(90deg,rgba(10,10,11,0.85),transparent 60%)" }}
        />
        ...
        <div className="absolute bottom-12 left-5 right-5 pl-0 pr-6 py-6 leading-[normal] pointer-events-none min-[681px]:left-[max(48px,calc((100%-1320px)/2))] min-[681px]:right-auto min-[681px]:max-w-[440px] min-[681px]:pr-[38px] min-[681px]:py-8">
          ...
          <Link href={cta?.href ?? "#"} className="inline-block text-inherit pointer-events-auto">
            <h2 className="font-grotesk font-medium text-3xl leading-[1.05] min-[681px]:text-[36px] tracking-[-0.03em]">
              {headline}
            </h2>
          </Link>
          {cta && (
            <Link
              href={cta.href}
              className="mt-4 inline-block pointer-events-auto font-mono text-[10.5px] tracking-[0.22em] text-white/75 transition-colors hover:text-mg-accentSerif"
            >
              {cta.label}
            </Link>
          )}
    ...
    <section className="container-mg py-16 md:py-24">
      <div className="grid md:grid-cols-2 gap-10 md:gap-16 items-center">
        ...
        <div className={imgFirst ? "md:order-2" : "md:order-1"}>
          ...
          <h2 className="font-grotesk font-semibold text-3xl md:text-5xl leading-[1.05] text-balance">
            {headline}
          </h2>
          {body && <p className="mt-6 text-mg-fg/70 text-pretty max-w-md">{body}</p>}
          {cta && (
            <div className="mt-8">
              <Button href={cta.href} variant={cta.style || "outline"}>
                {cta.label}
              </Button>
            </div>
          )}
