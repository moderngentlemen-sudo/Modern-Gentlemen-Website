...
export function PullQuote({ quote, attribution, size = "lg" }: Props) {
  return (
    <section
      className="py-[96px] text-center"
      style={{ paddingInline: "max(22px, calc((100% - 900px) / 2))" }}
    >
      <blockquote
        className={clsx(
          "font-serif italic text-mg-fg text-balance",
          size === "lg"
            ? "text-[27px] min-[681px]:text-[40px] leading-[1.24]"
      ...
      <div className="mt-6 font-mono text-[10px] uppercase tracking-[0.2em] text-mg-fg/60">
        {attribution}
      </div>
    </section>
  );
