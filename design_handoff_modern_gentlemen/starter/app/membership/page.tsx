...
const TIERS: Tier[] = [
  ...
  },
  {
    ...
    perks: ["Everything in The Reader", "The complete archive", "Member-only films & essays", "15% off the MG Shop"],
  ...
  },
  {
    ...
    perks: ["Everything in The Member", "The printed quarterly", "Invitations to MG events", "Early access to drops"],
    cta: "CHOOSE PLAN",
  },
];
...
const BENEFITS = [
  ...
  { no: "03", title: "Member Films", dek: "Extended cuts and films we don’t publish anywhere else." },
  { no: "04", title: "15% Off", dek: "A standing discount across everything in the shop." },
];

const FAQ = [
  { q: "Can I cancel anytime?", a: "Yes — memberships are month-to-month or annual, and you can cancel in a click from your account. No calls, no retention scripts." },
  { q: "What’s in the archive?", a: "Every article and film we’ve published since issue 001, plus member-only long reads that never appear on the open site." },
  { q: "Is there a free trial?", a: "The Member tier starts with seven days free. Cancel before it ends and you won’t be charged." },
  { q: "Does the shop discount stack?", a: "The 15% member discount applies to every order and combines with free shipping over £50." },
];
...
export default function MembershipPage() {
  ...
  return (
    <>
          ...
          {TIERS.map((t) => {
            ...
            return (
              ...
              >
                ...
                <div className={clsx("font-mono text-[10px] uppercase tracking-[0.2em]", t.featured ? "text-white/85" : "text-[#ff4d5e]")}>{t.name}</div>
                <div className="mt-[18px] flex items-end gap-[6px]">
                  <span className={clsx("font-grotesk font-semibold text-[46px] leading-none tracking-[-0.04em]", t.featured ? "text-white" : "text-mg-fg")}>{price}</span>
                  {per && <span className={clsx("mb-2 font-mono text-[11px]", t.featured ? "text-white/75" : "text-mg-fg/50")}>{per}</span>}
                </div>
                <p className={clsx("mb-[22px] mt-4 font-grotesk font-light text-[14px] leading-[1.55]", t.featured ? "text-white/85" : "text-mg-fg/60")}>{t.blurb}</p>
                <div className="mb-7 flex flex-col gap-3">
                  {t.perks.map((pk) => (
                    <div key={pk} className={clsx("flex items-start gap-[10px] font-grotesk font-light text-[14px] leading-[1.4]", t.featured ? "text-white/90" : "text-mg-fg/[0.82]")}>
                      <span className={clsx("mt-px flex-shrink-0", t.featured ? "text-white" : "text-mg-accent")} aria-hidden>
                        ✓
                      </span>
                      <span>{pk}</span>
                    </div>
      ...
      {/* Benefits */}
      <section className="pt-[88px]" style={{ paddingInline: "max(22px, calc((100% - 1320px) / 2))" }}>
        ...
        <HairlineGrid className="grid-cols-1 min-[681px]:grid-cols-2 min-[821px]:grid-cols-4">
          {BENEFITS.map((b) => (
            <div key={b.no} className="bg-mg-surface p-[34px_28px]">
              <div className="font-grotesk font-semibold text-[30px] leading-none tracking-[-0.03em] text-mg-accent">{b.no}</div>
              <div className="mt-4 font-grotesk font-medium text-[17px] leading-[1.15] tracking-[-0.015em]">{b.title}</div>
              <p className="mt-[10px] font-grotesk font-light text-[13.5px] leading-[1.55] text-mg-fg/[0.58] text-pretty">{b.dek}</p>
            </div>
          ))}
        </HairlineGrid>
      </section>
          ...
          {FAQ.map((f) => (
            <details key={f.q} className="group border-b border-mg-bd/10">
              <summary className="flex cursor-pointer list-none items-center justify-between gap-4 py-[22px] font-grotesk font-medium text-[19px] leading-[1.2] tracking-[-0.015em] [&::-webkit-details-marker]:hidden">
                <span>{f.q}</span>
                <span className="font-grotesk text-[22px] text-mg-accent group-open:hidden" aria-hidden>+</span>
                <span className="hidden font-grotesk text-[22px] text-mg-accent group-open:inline" aria-hidden>–</span>
              </summary>
              <p className="mb-[22px] font-grotesk font-light text-[15px] leading-[1.65] text-mg-fg/[0.66] text-pretty">{f.a}</p>
            </details>
          ))}
        </div>
      </section>

      <CtaBand variant="centered" gutter={22} heading="Considered, not hurried. Start this Sunday." buttonLabel="JOIN NOW" successLabel="WELCOME ✓" />
    </>
  );
}
