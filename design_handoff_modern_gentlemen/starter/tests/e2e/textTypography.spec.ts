...
test("text typography survives saving and renders on the published page", async ({ page }) => {
  ...
  await page.getByLabel("Font family", { exact: true }).selectOption("systemSerif");
  ...
  await expect(page.getByLabel("Font family", { exact: true })).toHaveValue("systemSerif");
  ...
  await expect(text).toHaveCSS("font-family", /ui-serif/);
});
