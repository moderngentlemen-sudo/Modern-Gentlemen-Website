...
function renderTree(){let out='';walk(doc.children,(n,parent,depth)=>{out+=`<div class="tree-row${n.id===selected?' active':''}" style="margin-left:${depth*10}px"><button class="grip" data-drag-id="${n.id}" aria-label="Drag ${esc(n.label)}">⠿</button><button class="tree-select" data-select="${n.id}">${esc(n.label)}</button><small>${n.visibility==='hidden'?'off':n.type==='column'?'cell':''}</small></div>`;});$('structure').innerHTML=out||'<div class="empty">Your canvas is empty.<br>Add a block from the library.</div>';}
...
async function exportDialog(){await ready;modal('Export & install',`<p class="modal-note">Text and links remain live in HTML. PNG is a flat visual. Selection outlines and drag controls are never exported.</p><div class="export-preview">${html()}</div>${/src="data:/.test(html())?'<div class="warning-box">Some artwork is local. Publish it before using Gmail. Published signature images are public, so recipients can load them.</div>':''}${prepared.errors.length?'<div class="warning-box">'+prepared.errors.map(esc).join('<br>')+'</div>':''}<div class="steps"><div><b>1</b><span>Prepare your artwork. ${/src="data:/.test(html())?'Publish images below.':'Hosted images are ready.'}</span></div><div><b>2</b><span>Copy the formatted signature, then open Gmail settings.</span></div><div><b>3</b><span>In General → Signature, create or select a signature, paste, choose defaults and save changes.</span></div></div><div class="modal-actions"><button data-action="publish">Publish images</button><button data-action="copy" class="primary">Copy formatted signature</button><a href="https://mail.google.com/mail/u/0/#settings/general" target="_blank" rel="noopener noreferrer">Open Gmail settings ↗</a></div><div class="mini-actions"><button data-action="export-html">Download HTML</button><button data-action="export-png">PNG · 2×</button><button data-action="backup">Editable project</button></div><p class="hint">Installing is always explicit. Saving or moving blocks never changes an installed Gmail signature. Direct OAuth setup is unchanged by this release.</p>`);}
async function copySignature(){await ready;if(prepared.errors.length)throw new Error(prepared.errors[0]);const markup=html();if(/src="data:/.test(markup))throw new Error('Publish local artwork before copying to Gmail, or use portable HTML export.');const plain=document.createElement('div');plain.innerHTML=markup;
 try{await navigator.clipboard.write([new ClipboardItem({'text/html':new Blob([markup],{type:'text/html'}),'text/plain':new Blob([plain.textContent],{type:'text/plain'})})]);toast('Formatted signature copied. Paste it in Gmail’s signature editor.');}
 catch{plain.style.position='fixed';plain.style.left='-9000px';document.body.append(plain);const range=document.createRange();range.selectNodeContents(plain);const selection=getSelection();selection.removeAllRanges();selection.addRange(range);const ok=document.execCommand('copy');selection.removeAllRanges();plain.remove();if(!ok)throw new Error('Clipboard permission was denied. Download HTML, open it in your browser, then select and copy the rendered signature.');toast('Signature copied.');}
}
...
const actions={
 ...
 duplicate:()=>{const r=duplicate(doc,selected);selected=r.id;commit(r.document);},delete:()=>{if(!selected)return;commit(remove(doc,selected));},
 ...
 export:exportDialog,publish:async()=>{if(await publish())await exportDialog();},copy:copySignature,
