...
import {layerBadge,toggleVisibilityState,documentForVariant} from '../shared/signature-rules.mjs';
...
function renderTree(){let out='';walk(doc.children,(n,parent,depth)=>{const visible=n.visibility!=='hidden',tag=layerBadge(n);out+=`<div class="tree-row${n.id===selected?' active':''}" style="margin-left:${depth*10}px"><button class="grip" data-drag-id="${n.id}" aria-label="Drag ${esc(n.label)}">⠿</button><button class="tree-select" data-select="${n.id}">${esc(n.label)}</button><button data-action="toggle-visibility" data-id="${n.id}" aria-label="${visible?'Hide':'Show'} ${esc(n.label)}" title="${visible?'Hide':'Show'} ${esc(n.label)}">${visible?'◉':'○'}</button><small>${esc(tag)}</small></div>`;});$('structure').innerHTML=out||'<div class="empty">Your canvas is empty.<br>Add a block from the library.</div>';}
...
function variantHtml(variant){
 const version=documentForVariant(doc,variant);
...
async function copyVariant(variant){await ready;const markup=variantHtml(variant);return copyMarkup(markup,variant==='reply'?'Reply signature':'Full signature');}
...
const actions={
 ...
 duplicate:()=>{const r=duplicate(doc,selected);selected=r.id;commit(r.document);},delete:()=>{if(!selected)return;commit(remove(doc,selected));},'toggle-visibility':b=>{const id=b.dataset.id||selected;if(!id)return;mutate(d=>{const n=find(d,id);if(n)toggleVisibilityState(n);},'visibility:'+id);},
